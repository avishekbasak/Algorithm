...
def bfs(tree):
    ...
    while len(q) >0:
        ...
        val = q.deq()
        #visit the node
        visit_order.append(val.get_value())
        #check for left child and add the same
        if val.has_left_child():
            q.enq(val.get_left_child())
        #check for right child and add the same   
        if val.has_right_child():
            q.enq(val.get_right_child())
    #return the order
    return visit_order
